<?php
    ...
    $ordenes = new Fechas();
    $datosOrden = $ordenes->fechasRangoCli($tabla, $desde, $hasta);
    $totalRegistros = @mysqli_num_rows($datosOrden);
    $fpdf->Cell(195, 8, 'Total de Clientes ('.$totalRegistros.')', 0, 1, 'R');

    $fpdf->SetFillColor(173, 181, 189);
    $fpdf->SetFont('Arial', 'B', 10);
    $fpdf->Cell(15, 8, '#', 1, 0, 'C', 1);
    $fpdf->Cell(40, 8, 'Total', 1, 0, 'C', 1);
    $fpdf->Cell(50, 8, 'Estatus', 1, 0, 'C', 1);
    $fpdf->Cell(40, 8, 'Registro', 1, 1, 'C', 1);

    $fpdf->SetFont('Arial', '', 10);
    
    while ($resultado = mysqli_fetch_array($datosOrden)) {
        $fpdf->Cell(15, 8, $resultado['id_orden'], 1, 0, 'C');
        $fpdf->Cell(40, 8, utf8_decode($resultado['id_orden']), 1, 0, 'C');
        $fpdf->Cell(50, 8, $resultado['id_orden'], 1, 0, 'C');
