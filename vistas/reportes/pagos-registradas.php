<?php
    ...
    $fpdf->Cell(75);
    $fpdf->Cell(50, 5, 'Pagos Registrados', 0, 1, 'C');
    $fpdf->Ln(5);
    $fpdf->Cell(75);
    ...
    $fpdf->Cell(195, 8, 'Total de pagos registrados('.$totalRegistros.')', 0, 1, 'R');

    $fpdf->SetFillColor(173, 181, 189);
    $fpdf->SetFont('Arial', 'B', 10);
    $fpdf->Cell(25, 8, '#', 1, 0, 'C', 1);
    $fpdf->Cell(50, 8, 'Nombre', 1, 0, 'C', 1);
    $fpdf->Cell(30, 8, 'Referencia', 1, 0, 'C', 1);
    ...
    $fpdf->SetFont('Arial', '', 10);
        ...
        while ($info_pago = mysqli_fetch_array($datos)) {
            $fpdf->Cell(25, 8, $info_pago['id_pago'], 1, 0, 'C');
            $fpdf->Cell(50, 8, utf8_decode($info_pago['nombre']), 1, 0, 'C');
            $fpdf->Cell(30, 8, $info_pago['referencia_p'], 1, 0, 'C');
            $fpdf->Cell(30, 8, $info_pago['estatus_p'], 1, 0, 'C');
            $fpdf->Cell(60, 8, $fecha->fechaFormato($info_pago['fecha_registro']), 1, 1, 'C');
        }
        


    $fpdf->OutPut('I', 'Pagos Registradas.pdf', true);
